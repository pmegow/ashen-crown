...
// #302 (owner ruling 2026-09-03): progression is ENGINE-PAID and level-scaled. The GM's own [XP:]
// is flavour, capped per response; the paymaster is awardMilestoneXp (tag_table.js), fired when
// the structured milestone lands — a quest close, a boss slain at a victory close, an act close.
...
// (a market belongs to the settlement, not the tavern the party stands in), capped by the node's
// LOCATION_SIZE tier — the GM's size vocabulary is physical scale (small/medium/large/vast), so
// the cap keys off that — and they EXPIRE after WARES_RESTOCK_DAYS of game time, when the market
// ask (buildMarketNote, api.js) fires again. Price = VALUE, never the party's purse: a [WARES:]
// price outside WARES_PRICE_BAND× of the bible value is warned about, never rewritten.
var WARES_RESTOCK_DAYS=7;
...
var PHASE_MISMATCH_MIN=240;
var PHASE_CHECK_GRACE_MIN=60;   // #387: a [TIME_CHECK:] outside its own band by more than this arms the reconcile nudge (the 240 above is the fuzzy PROSE recogniser's tolerance)  // #158: minutes of BAND distance between a narrated phase assertion and the clock before the GM-decides reconcile nudge arms (clockPhaseDetect, clock.js). 4h swallows honest estimation slop and adjacent-phase wording; the t1605 class (dusk narrated at 11:10 am = 7h+) clears it with room
