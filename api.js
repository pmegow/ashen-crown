...
/* #225 (field t2228-2231, the Bronze Bell Warden ghost fight): the INVERSE of #214② below.
   That one asks when an OPEN encounter goes untagged; this one asks when combat-action tags
   arrive with NO encounter — [COMBAT_END:fled] closed the fight, the story re-engaged the foe,
   and four turns of ENEMY_HP/ENEMY_SLAIN/COMBAT_END no-opped console-only while the GM narrated
   an hp bar that existed nowhere. Table Talk, asked "how is it not dead at 0 hp", correctly
   answered that no state exists — TT's honesty exposed the gap; this is the engine's half.
   Deliberately NOT combat-silent (the #214② precedent): its subject IS the missing encounter.
   Moot-aware: if combat is open again at delivery time, the GM already fixed it — clear silently. */
function buildOrphanCombatNudge(){
  if(!worldState)return"";
  var q=worldState.orphanCombat;
  if(!q||q.delivered)return"";
  if(worldState.combat){delete worldState.orphanCombat;return"";}
  q.delivered=true;
  return "[ENGINE NOTE — COMBAT TAGS WITHOUT AN ENCOUNTER (not a player action): your recent combat tags ("+q.tags.join(", ")+") arrived after the encounter was CLOSED and applied NOTHING — no damage, no kill, no round was recorded. If the fight resumed after the close, RE-OPEN it now with [COMBAT_START:name|hp|ac|atkbonus|dmgdie|morale] using the foe's remaining hp as its pool, then re-assert the outcome ([ENEMY_HP:]/[ENEMY_SLAIN:]) so the record catches up. If the fight is genuinely over, narrate freely and emit no combat tags.]";
}
...
function buildPlotArmorNote(){
  var p=worldState&&worldState.plotArmorPing;if(!p||!p.name)return"";
  delete worldState.plotArmorPing;
  return "[ENGINE NOTE \u2014 PLOT ARMOR (not a player action): the death of "+p.name+" was REFUSED \u2014 they are load-bearing until Act "+p.act+(p.arc?" \u201c"+p.arc+"\u201d":"")+" and cannot die yet (escape "+p.escapes+" of "+p.max+"). THIS response, without contradicting what the player saw: stage the exit \u2014 the body is never found, a leap over the railing, a rescue at the last instant, a capture alive \u2014 and make it COST "+p.name+": they leave behind something the player takes (a key, a name, a wound, a lead). Do not narrate "+p.name+" dead again; the record says they got away.]";
}
...
// tag, or [LOCATION_HOURS:none]. Combat-silent; latch worldState.hoursAsk={node,turn}.
function buildHoursNote(){
  if(!worldState||worldState.combat||typeof memory==="undefined"||!memory||!memory.map||!worldState.world||!worldState.world.location||!worldState.world.sublocation)return"";
  var wkey=worldState.world.location;if(typeof locResolve==="function")wkey=locResolve(wkey);var wnode=memory.map.nodes[wkey];if(!wnode||!wnode.size)return"";
  ...
  var node=memory.map.nodes[key];if(!node||node.hours||node.hoursNone)return"";
  var ha=worldState.hoursAsk;if(ha&&ha.node===key)return"";
  worldState.hoursAsk={node:key,turn:worldState.turn};
  var label=(typeof locDisplayLeaf==="function")?locDisplayLeaf(key):key;
  return "[ENGINE NOTE \u2014 HOURS (not a player action): "+label+" is new to the record. If it is an establishment that keeps hours (a shop, a temple, a gate, a tavern, an office), file them ONCE this response: [LOCATION_HOURS:open-close|note] \u2014 whole hours 0-24, overnight ranges like 20-4 are fine. If it never closes, or is not that kind of place, reply [LOCATION_HOURS:none]. The clock then says OPEN or CLOSED on every later visit; do not mention this note.]";
}
...
function buildMontageNote(){
  var r=worldState&&worldState.montagePing;if(!r)return"";delete worldState.montagePing;
  return "[ENGINE NOTE — MONTAGE (not a player action): the player asked to skip ahead. Write ONE paragraph that compresses the stretch to come: hours pass, the routine work gets done, at most two or three summarized checks ([SKILL_SUCCESS:] as earned), and the passage LANDS at the next real decision — a person arriving, a discovery, a fork in the road — where you stop and hand the choice back. Advance the clock honestly with [TIME_ADVANCE:] for the hours skipped. No new named NPCs, no combat, no travel to another world location inside the montage.]";
}
// #308 ②: the WRAP-UP contract — Car Mode's "wrap up": land the scene at a hook within two responses.
function buildWrapUpNote(){
  var r=worldState&&worldState.wrapUpPing;if(!r)return"";delete worldState.wrapUpPing;
  return "[ENGINE NOTE — WRAP UP (not a player action): the player is about to stop for now. Within two responses, bring the scene to a natural stopping point — finish the fight if one is open, close the conversation, and END on a hook: a clear choice for next time, stated plainly in the last sentence. Do not start anything new that cannot be left.]";
}
...
// #355: the REGISTER census + note. registerFile runs on the cleaned narration after every turn
// (observeDriftAxes); a hit files the record ring (capped, save-round-trips like diceLog) and arms
// registerPing. The note fires ONCE on the next turn, naming the word — silent in combat (the fight
...
function registerFile(clean,turn){
  ...
  worldState.registerPing={words:hits,turn:turn};
  return hits;
}
function buildRegisterNote(){
  var p=worldState&&worldState.registerPing;if(!p)return"";
  if(worldState.combat)return"";/* silent in combat — the latch waits */
  delete worldState.registerPing;
  var w=(p.words||[]).map(function(x){return "'"+x+"'";}).join(", ");
  return "[ENGINE NOTE \u2014 REGISTER (not a player action): the last narration used "+w+". This game's fiction is never clerical \u2014 no ledgers, invoices, paperwork, bookkeeping or accountants as image, metaphor or plot device. Debts here are blood, oaths, hunger and memory. Do not repeat the word and do not build on the image; say what the thing IS.]\n";
}
// #344: the SUGGEST-miss note. generateActions arms suggestMissPing when the in-band setting is on and the
// GM's response arrived without [SUGGEST:] (the v1.288 second call then ran as the honest floor). The next
// turn carries a one-line ask beside the model's own recent output — the "new channel" lesson: sonnet-5
// wrote every other tag on 0/50 turns and never this one while the ask lived only in the tag doc.
// #391: the player backed out of a named roll (the #371 stake made the choice real — owner, The Long Walk t131:
// "a risk has been announced, but there doesn't seem to be a way to back out"). One-shot, fires in combat too
// (a check can be a combat check): the GM must narrate the hold-back from the player's next action and never
// roll or resolve the withdrawn check itself. Armed by reconsiderPendingCheck (game.js), delivered once.
function buildCheckWithdrawnNote(){
  var p=worldState&&worldState.checkWithdrawnPing;if(!p||!p.label)return"";
  delete worldState.checkWithdrawnPing;
  return "[ENGINE NOTE \u2014 CHECK WITHDRAWN (not a player action): the player declined the "+p.label+(p.dc!=null?" (DC "+p.dc+")":"")+" after the stake was named \u2014 the attempt was NOT made and no die was rolled. Narrate what they do instead from their action below; never roll or resolve that check for them, and never treat the hold-back as a failure.]\n";
}
function buildSuggestMissNote(){
  var p=worldState&&worldState.suggestMissPing;if(!p)return"";
  if(typeof suggestInband!=="undefined"&&!suggestInband){delete worldState.suggestMissPing;return"";}
  delete worldState.suggestMissPing;
  return "[ENGINE NOTE \u2014 SUGGESTED ACTIONS MISSING (not a player action): your last response ended without the [SUGGEST:a|b|c] tag, so the player's buttons had to be generated separately. END this response, and every narrative response, with [SUGGEST:action one|action two|action three] \u2014 exactly three short actions \u2014 after the prose and every other tag.]\n";
}
...
function buildDownedNote(){
  var d=worldState&&worldState.downed;if(!d||worldState.deathPending)return"";
  var c=worldState.character,who=(typeof presentCompanions==="function")?presentCompanions().map(function(n){return n.name;}):[],turns=d.turns||0,last=turns>=DOWNED_MAX_TURNS-1;
  return "[ENGINE NOTE — PLAYER DOWNED (not a player action): "+c.name+" is at 0 HP — DOWN, not dead. The engine has taken the wheel: the player may only STRUGGLE or YIELD this turn. "+(who.length?"Present and able to intervene: "+who.join(", ")+". ":"No companion is present to intervene. ")+(last?"Resolve it in THIS response — ":"Resolve it within two responses — ")+"capture, rescue, a companion's intervention, or a stranger's mercy — and mark the outcome [DOWNED_RESOLVED:captured|why] / [DOWNED_RESOLVED:rescued|why] / [DOWNED_RESOLVED:intervened|why], healing with [HP:+N] as the story allows; the engine files the scar. If the story truly kills them, [DOWNED_RESOLVED:dead|how]. Left unresolved for "+DOWNED_MAX_TURNS+" turns, the engine rules a death itself.]";
}
...
function buildIdentityConflictNudge(){
  ...
  /* #201: teach only REACHABLE ceremonies. The t2032 deadlock: the nudge kept teaching the reveal
     of a handle whose frame had sealed and summarized away — a ceremony that provably cannot
     succeed. When the record's handle resolves to NO live actor, the reveal is off the menu:
     witnessed evidence under the subject's own name authorizes directly (the road the Caul and
     Golvak repairs took), and failing that, [NPC_DEATH_REPORTED:] is the honest off-screen valve. */
  var liveHandle=handle&&typeof _sceneRefActor==="function"&&!!_sceneRefActor(c.handle);
  var ceremony;
  if(handle&&known&&liveHandle)ceremony="If an on-screen reveal genuinely establishes the handle's identity, emit [SCENE_REVEAL:"+c.handle+"|"+c.subject+"] now; put the death and every caused objective/reward inside one CANON_TXN with a NEW stable claim id on the NEXT response";
  else if(handle&&known&&typeof w2NamedPresenceEvidence==="function"&&w2NamedPresenceEvidence(c.subject))ceremony="The handle \""+c.handle+"\" no longer resolves to anyone on the record — do NOT try to reveal it. "+c.subject+" has witnessed evidence under their OWN name: re-emit the death inside one CANON_TXN with a NEW stable claim id, using "+c.subject+" as BOTH the subject and the scene handle ([SCENE_DEATH:"+c.subject+"]), with every caused objective/reward inside the same envelope";
  else if(handle&&known)ceremony="The handle \""+c.handle+"\" no longer resolves to anyone on the record and no witnessed evidence supports the name — if the death truly happened, emit [NPC_DEATH_REPORTED:"+c.subject+"|how the party learned of it] in THIS response (the engine commits it honestly as a reported death; owed rewards then ride a new CANON_TXN)";
...
var buildPhaseMismatchNudge=oneShotPing("phaseMismatch",{name:"buildPhaseMismatchNudge",moot:function(q){return typeof clockPhaseBandDist==="function"&&clockPhaseBandDist(q.idx)<(q.src==="check"&&typeof PHASE_CHECK_GRACE_MIN==="number"?PHASE_CHECK_GRACE_MIN:PHASE_MISMATCH_MIN);},text:function(q){
  if(q.src==="check")return "[ENGINE NOTE — CLOCK vs YOUR DECLARATION (not a player action): you opened with [TIME_CHECK:"+q.label+"] but the campaign clock reads "+q.stamp+(q.clockPhase?" — "+q.clockPhase:"")+". The clock is the record. Declare what the clock shows, and write the scene at that hour; if the story has genuinely moved to "+q.label+", emit [TIME:"+q.label+"] and the engine will reconcile. Never copy your previous declaration.]";/* #387 */
...
function buildSkeletonBlock(){
  ...
  if(worldState.blueprintName){if(typeof codaState==="function"&&codaState())lines.push("AUTHORED CAMPAIGN — this is \""+worldState.blueprintName+"\", a pre-written adventure the player deliberately chose, and its AUTHORED SPINE below is complete. Keep faith with what it established; steer toward nothing — there is no current arc. Use the character's backstory, flaw, and personality to COLOR what the party does with the world it made.");/* #366b: the fidelity line in a coda */
...
function diceTxt(t){var out="",di;
  ...
  var cs=String(t||"").match(/\[CHECK:([^\]]+)\]/g)||[];for(di=0;di<cs.length;di++){var cm=cs[di].match(/\[CHECK:([^\]]+)\]/),ck=cm&&typeof parseCheckTag==="function"?parseCheckTag(cm[1]):null;if(!ck)continue;
    ...
    out+=live?'<div class="dice-block dice-pending" onclick="rollPendingCheck()" title="Click to roll">'+lbl+'d20'+(ck.mod?(ck.mod>0?"+":"")+ck.mod:"")+(ck.dc!=null?' vs DC '+ck.dc:'')+' &middot; <strong class="roll-due">click to roll</strong> <span class="roll-alt" onclick="event.stopPropagation();reconsiderPendingCheck()" title="Back out: no roll is made; say what you do instead">&middot; or reconsider</span></div>'/* #391: the way out is on the card */:'<div class="dice-block" style="opacity:.6;">'+lbl+'d20 &middot; unrolled</div>';}
