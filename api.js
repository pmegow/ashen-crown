...
function diceTxt(t){var out="",di;
  ...
  var cs=String(t||"").match(/\[CHECK:([^\]]+)\]/g)||[];for(di=0;di<cs.length;di++){var cm=cs[di].match(/\[CHECK:([^\]]+)\]/),ck=cm&&typeof parseCheckTag==="function"?parseCheckTag(cm[1]):null;if(!ck)continue;
    ...
    out+=live?'<div class="dice-block dice-pending" onclick="rollPendingCheck()" title="Click to roll">'+lbl+'d20'+(ck.mod?(ck.mod>0?"+":"")+ck.mod:"")+(ck.dc!=null?' vs DC '+ck.dc:'')+' &middot; <strong class="roll-due">click to roll</strong></div>':'<div class="dice-block" style="opacity:.6;">'+lbl+'d20 &middot; unrolled</div>';}
