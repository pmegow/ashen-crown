...
rc|=sabotage.prove({
  ...
  cases:[
    {label:"a victory close goes back to discarding foes that are still standing (#214)",
      ...
      /* #268 re-point: recognition became defeat-exclusion-first + victory-markers-anywhere */
      find:"    if(worldState.combat&&_ceVictory){",
      replace:"    if(false){"},
    {label:"the outcome word stops being checked, so a rout and a retreat kill alike (#214)",
      ...
      find:"var _ceVictory=!_ceDefeatLed&&",
      replace:"var _ceVictory=true||!_ceDefeatLed&&"},
    {label:"#268: the defeat exclusion dies — a mixed 'defeat — the warden kills the party' executes the foes",
      mustFail:"NEVER reads as a win",
      find:"var _ceDefeatLed=/(defeat|",
      replace:"var _ceDefeatLed=false&&/(defeat|"},
    {label:"#268: the unrecognized-outcome fork dies — living foes are silently discarded again",
      mustFail:"UNRECOGNIZED outcome over living foes is loud",
      find:"    if(worldState.combat&&!_ceVictory&&!_ceDefeatLed&&_ceRest.length){",
      replace:"    if(false){"},
