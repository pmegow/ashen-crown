...
rc |= sabotage.prove({
  ...
  /* un-skipped at the Fable merge: #264 (v1.733) repaired the gate — run-tests' tail is now
     process.exit(process.exitCode||0), so the #213 contract can fail the build. */
  command: GATE,
  cases: [
    {
      label: "#213 a new refusal reason ships with no player sentence in the registry",
      /* re-pointed at the Fable merge: Sol's #279 census detects shipped reasons BY SYNTAX
         (reason assignments), so the old bare-var mutation no longer registers as shipped.
         Mutating a REAL assignment to unmapped text is the honest modern shape. The chosen
         reason is deliberately NOT one of the three literals the joint-f24 engine test
         byte-pins — that red would misattribute. */
      mustFail: "REFUSAL COPY",
      find: "reason=\"summary death lacks a source turn\"",
      replace: "reason=\"zz wholly unmapped summary refusal zz\""
    },
    {
