...
function applySummaryExtract(extracted,identityTable){
  ...
  /* #388: attire belt — the player is NOT rejected here (npcUpdates rejects the player by design; attire is sheet
     state on every party member). Same helpers as the live tags: nothing worn that is not carried, outfit replaced. */
  if(Array.isArray(extracted.attire)&&typeof attireSheet==="function"){for(i=0;i<extracted.attire.length;i++){var _at=extracted.attire[i];if(!_at||!_at.name)continue;var _atCs=attireSheet(_at.name);
    if(!_atCs){if(typeof console!=="undefined")console.warn("[attire] extractor named '"+_at.name+"' — no party member by that name; dropped (#388)");continue;}
    var _atJ;if(Array.isArray(_at.doffed))for(_atJ=0;_atJ<_at.doffed.length;_atJ++)if(typeof _at.doffed[_atJ]==="string")wornSet(_atCs,_at.doffed[_atJ],false,_at.name);
    if(Array.isArray(_at.donned))for(_atJ=0;_atJ<_at.donned.length;_atJ++)if(typeof _at.donned[_atJ]==="string")wornSet(_atCs,_at.donned[_atJ],true,_at.name);
