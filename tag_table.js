...
{t:"REST",apply:function(text,R){if(/\[REST:\s*long\b[^\]]*\]/i.test(text)&&typeof restSpells==="function"){var _sleepMode=(typeof clockSleepMode==="function")?clockSleepMode():"dawn";/* #346: read before the roll */var _slept=restSpells(true);/* #89: restSpells owns the dawn roll — one site, both paths (button + tag); #300: it heals to full and, from the tag path, QUEUES the camp (taken at commit, never mid-parse) */R.muts.push("Rest: healed to full, spell slots restored"+(_slept?"; "+(_sleepMode==="fixed"?"slept eight hours":"slept until dawn")+" (+"+_slept+"m, "+clockFmt()+")":""));worldState.checkpointDue="rest";}
