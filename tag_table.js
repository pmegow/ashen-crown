...
"[WORN:Name|item|on] / [WORN:Name|item|off] -- a carried item is put on or taken off (armor buckled, shield slung, a ring slipped on; armor stripped for the night, sent out for laundry). Name is the player or a party member; the item must already be in their inventory (an uncarried item is refused). [OUTFIT:Name|one line] -- what they wear beneath or instead of gear, ONLY when it changes (stripped for a bath, wrapped in a borrowed robe, back in road clothes); a dozen words, replaced each time. The engine serves both back on the sheet as Wearing: -- keep the prose consistent with it, or correct it with a new tag\n",/* #388 */
...
"[COMPANION_AGENDA:Name|the want in one sentence|violent or peaceful] -- a companion's WANT of their own (see COMPANION AGENDAS): file ONE at their introduction when the engine asks, or when a defining moment births one (the engine asks). The first becomes their active want; a later one waits SILENT until the active one resolves\n",
"[COMPANION_AGENDA_DONE:Name] -- the companion's active want is fulfilled (their next silent want, if any, becomes active -- the engine will ask you to announce it). [COMPANION_AGENDA_DONE:Name|a few words of a silent want] -- the story resolved a silent want early; give it one line of acknowledgement\n",
"[COMPANION_AGENDA_BEAT:Name] -- you played a beat of the companion's active want this response (a remark, a detour proposed, a plan volunteered, a refusal); it resets the engine's push clock\n",
"[COMPANION_GROWTH:Name|the flaw as written on their sheet|what replaced it] -- RARE: the companion acted AGAINST their flaw at a defining moment, on screen and unasked (the spider-fearing companion who drags the party out of the web). The sheet's flaw is rewritten and the old one kept. Never file growth because the engine or the player asked for it\n",/* #370 */
"[COMPANION_INITIATIVE:Name|what they did, one line] -- a companion acted on a compulsion-shaped flaw UNBIDDEN this response (pocketed what glittered, froze before the priest, blurted the truth); file it beside the roll and the consequence on their own sheet. Rare; never on a turn the player already stepped in\n",/* #386 */
...
"A danger the party knowingly walks away from is also a FUTURE_EVENT (when: left unresolved) -- file it so it can return later, unasked.\n",/* #369 */
...
"[COMPANION_SKILL_SUCCESS:Name|skill_id] -- the same for a named party member's OWN successful skilled action: companions earn their own ladder (never file a companion's success under the player)\n",
...
/* #357: THE skill-success applier — one increment path for the hero and every companion. Resolves a
   lowercased id (E29), initialises a missing map, bumps the counter, and announces a ladder step
   (mut + toast). `who` is null for the hero, the companion's name otherwise (prefixes the mut/toast). */
function skillSuccessApply(sheet,rawId,R,who,tagName){
  if(!sheet)return false;var id=String(rawId||"").trim();if(!sheet.skills)sheet.skills=initSkills();
  if(typeof sheet.skills[id]!=="number"){var _l=id.toLowerCase(),_i;for(_i=0;_i<SKILLS.length;_i++){if(SKILLS[_i].id.toLowerCase()===_l){id=SKILLS[_i].id;break;}}}
  if(typeof sheet.skills[id]!=="number"){if(typeof console!=="undefined")console.warn("[tags] "+(tagName||"SKILL_SUCCESS")+": unknown skill id '"+id+"' — no increment (#136③)");return false;}
  var pfx=who?who+" — ":"",prev=skillLevel(sheet.skills[id]);sheet.skills[id]++;var now=skillLevel(sheet.skills[id]);
  if(now>prev){R.muts.push(pfx+id+": "+SKILL_LEVELS[now]);showToast(pfx+id+": "+SKILL_LEVELS[now]);}else R.muts.push(pfx+id+" +1");
  return true;
}
var TAG_TABLE=[
...
/* #330 companions with agendas — the three writes. Placement is the ENGINE's (first want active, later ones silent),
   completion promotes and arms the announce note, a beat resets the push clock. Companion sheets only. */
...
{t:"COMPANION_AGENDA_DONE",apply:function(text,R){var ts=text.match(/\[COMPANION_AGENDA_DONE:([^|\]]+)(?:\|([^\]]*))?\]/g)||[],i;for(i=0;i<ts.length;i++){var m=ts[i].match(/\[COMPANION_AGENDA_DONE:([^|\]]+)(?:\|([^\]]*))?\]/);if(!m)continue;var nm=resolveNpcName(m[1].trim()),cs=findCompanionChar(nm);if(!cs)continue;
  ...
  R.muts.push(nm+": want fulfilled — "+res.done+(res.early?" (before its turn)":""));if(res.promoted){worldState.agendaAnnounce={name:nm,want:res.promoted,turn:R.turn};R.muts.push(nm+" now wants: "+res.promoted);}}}},
/* #386 companion initiative — the record of a flaw acting on its own. The roll and the consequence ride their own
   tags ([DICE:], [COMPANION_ITEM_GAINED:], [COMPANION_CONDITION:], an NPC attitude); this one stamps the sheet, fills the
   ring the prompt reads for its cadence, and toasts (#347: every filing is a visible event). Unknown companion: loud, ignored. */
{t:"COMPANION_INITIATIVE",apply:function(text,R){var ci=text.match(/\[COMPANION_INITIATIVE:([^|\]]+)\|([^\]]+)\]/g)||[],cii;for(cii=0;cii<ci.length;cii++){var cim=ci[cii].match(/\[COMPANION_INITIATIVE:([^|\]]+)\|([^\]]+)\]/);if(!cim)continue;var cin=resolveNpcName(cim[1].trim()),cics=findCompanionChar(cin);
  if(!cics){if(typeof console!=="undefined")console.warn("[initiative] [COMPANION_INITIATIVE:"+cim[1].trim()+"] — no companion sheet by that name; ignored (#386)");continue;}
  var cir=companionInitiativeFile(cics,cim[2],R.turn);if(!cir)continue;R.muts.push(cir.name+" acts on their own: "+cir.what);if(typeof showToast==="function")showToast("\u2605 "+cir.name+" acts on their own: "+cir.what);}}},
{t:"COMPANION_AGENDA_BEAT",apply:function(text,R){var ts=text.match(/\[COMPANION_AGENDA_BEAT:([^\]]+)\]/g)||[],i;for(i=0;i<ts.length;i++){var m=ts[i].match(/\[COMPANION_AGENDA_BEAT:([^\]]+)\]/);if(!m)continue;var cs=findCompanionChar(resolveNpcName(m[1].trim()));if(cs&&cs.agenda){cs.agenda.lastBeat=(typeof clockNow==="function")?clockNow():0;cs.agenda.beats=(cs.agenda.beats||0)+1;}}}},
...
{t:"ITEM_RENAMED",apply:function(text,R){var irTags=text.match(/\[ITEM_RENAMED:([^|\]]+)\|([^\]]+)\]/g)||[];var iri;for(iri=0;iri<irTags.length;iri++){var irm=irTags[iri].match(/\[ITEM_RENAMED:([^|\]]+)\|([^\]]+)\]/);if(!irm)continue;renameInventoryItem(worldState.character.inventory,irm[1].trim(),irm[2].trim(),R,null);if(typeof wornRename==="function")wornRename(worldState.character,irm[1].trim(),irm[2].trim());/* #388 */}}},
...
{t:"REST",apply:function(text,R){if(/\[REST:\s*long\b[^\]]*\]/i.test(text)&&typeof restSpells==="function"){var _wake=(typeof clockNamedWake==="function")?clockNamedWake(text):null;if(_wake)worldState.restWake=_wake;/* #389: the GM named the wake — transient, deleted below */var _sleepMode=(typeof clockSleepMode==="function")?clockSleepMode():"dawn";/* #346: read before the roll */var _slept;try{_slept=restSpells(true);}finally{delete worldState.restWake;}/* #89: restSpells owns the dawn roll — one site, both paths (button + tag); #300: it heals to full and, from the tag path, QUEUES the camp (taken at commit, never mid-parse) */R.muts.push("Rest: healed to full, spell slots restored"+(_slept?"; "+(_sleepMode==="fixed"?"slept eight hours":_sleepMode==="named"?"slept until "+_wake.label+" (named)":"slept until dawn")+" (+"+_slept+"m, "+clockFmt()+")":""));worldState.checkpointDue="rest";}
...
{t:"NPC_SUPERSEDE",apply:function(text,R){__tagNearMiss(text,R,"NPC_SUPERSEDE","^\\[NPC_SUPERSEDE:[^|\\]]*\\S[^|\\]]*\\|[^|\\]]*\\S[^|\\]]*\\|[^|\\]]*\\S[^|\\]]*\\]$","[NPC_SUPERSEDE:name|outdated|truth] — three non-blank fields");var sups=text.match(/\[NPC_SUPERSEDE:([^|\]]+)\|([^|]+)\|([^\]]+)\]/g)||[];var spi;for(spi=0;spi<sups.length;spi++){var spp=sups[spi].match(/\[NPC_SUPERSEDE:([^|\]]+)\|([^|]+)\|([^\]]+)\]/);if(!spp)continue;var spName=resolveNpcName(spp[1].trim()),spOld=spp[2].trim(),spNew=spp[3].trim();if(!spOld||!spNew)continue;
  ...
  var spNpc=memory.npcs[spName],spLow=spOld.toLowerCase(),spRet=[];if(authoredSupersede(spNpc,spOld)){fileNpcKnowledge(spName,spNew,R.turn,true);console.warn("[tags] authored dossier retained for "+spName+"; play fact filed alongside");R.muts.push(spName+": authored dossier retained; learned "+spNew);continue;}
...
{t:"SKILL_SUCCESS",apply:function(text,R){var skSuccs=text.match(/\[SKILL_SUCCESS:([^\]]+)\]/g)||[];var sski;for(sski=0;sski<skSuccs.length;sski++){var sskp=skSuccs[sski].match(/\[SKILL_SUCCESS:([^\]]+)\]/);if(!sskp)continue;skillSuccessApply(worldState.character,sskp[1],R,null,"SKILL_SUCCESS");}}},
/* #357: the companion twin — companions earn their OWN ladder (the audits' clearest partial implementation:
   SKILL_SUCCESS wrote only the hero's map, so a party member the player hopped control to never gained a
   bonus). Same applier, name-routed via findCompanionChar like every other COMPANION_* mirror. */
{t:"COMPANION_SKILL_SUCCESS",apply:function(text,R){var cSk=text.match(/\[COMPANION_SKILL_SUCCESS:([^|\]]+)\|([^\]]+)\]/g)||[];var cSki;for(cSki=0;cSki<cSk.length;cSki++){var cSkm=cSk[cSki].match(/\[COMPANION_SKILL_SUCCESS:([^|\]]+)\|([^\]]+)\]/);if(!cSkm)continue;var cSkCs=findCompanionChar(cSkm[1]);if(!cSkCs){if(typeof console!=="undefined")console.warn("[tags] no party member matches '"+cSkm[1].trim()+"' — companion tag dropped (#136③)");continue;}skillSuccessApply(cSkCs,cSkm[2],R,cSkCs.name||cSkm[1].trim(),"COMPANION_SKILL_SUCCESS");}}},
...
{t:"COMPANION_ITEM_RENAMED",apply:function(text,R){var crTags=text.match(/\[COMPANION_ITEM_RENAMED:([^|\]]+)\|([^|\]]+)\|([^\]]+)\]/g)||[];var cri;for(cri=0;cri<crTags.length;cri++){var crm=crTags[cri].match(/\[COMPANION_ITEM_RENAMED:([^|\]]+)\|([^|\]]+)\|([^\]]+)\]/);if(!crm)continue;var crOwner=crm[1].trim(),crCs=findCompanionChar(crOwner);if(!crCs){if(typeof console!=="undefined")console.warn("[tags] no party member matches '"+crOwner+"' — companion tag dropped (#136③)");continue;}if(!crCs.inventory)crCs.inventory=[];renameInventoryItem(crCs.inventory,crm[2].trim(),crm[3].trim(),R,crOwner);if(typeof wornRename==="function")wornRename(crCs,crm[2].trim(),crm[3].trim());/* #388 */}}},
/* #388: ATTIRE — name-addressed (the player by name, or a party member), ordered AFTER every ITEM_* handler so a
   same-response gain-then-wear lands. Nothing is worn that is not carried (wornSet refuses loudly, api.js). */
{t:"WORN",apply:function(text,R){var wt=text.match(/\[WORN:([^|\]]+)\|([^|\]]+)\|([^\]]+)\]/g)||[],wi;for(wi=0;wi<wt.length;wi++){var wm=wt[wi].match(/\[WORN:([^|\]]+)\|([^|\]]+)\|([^\]]+)\]/);if(!wm)continue;var wWho=wm[1].trim(),wCs=attireSheet(wWho);
  if(!wCs){if(typeof console!=="undefined")console.warn("[attire] [WORN:"+wWho+"] — no party member by that name; ignored (#388)");continue;}
  var wOn=/^(on|donned|worn|wear|true|yes)$/i.test(wm[3].trim()),wOff=/^(off|doffed|removed|remove|false|no)$/i.test(wm[3].trim());
  if(!wOn&&!wOff){if(typeof console!=="undefined")console.warn("[attire] [WORN:"+wWho+"|"+wm[2].trim()+"|"+wm[3].trim()+"] — the third field must be on or off; ignored (#388)");continue;}
  var wr=wornSet(wCs,wm[2].trim(),wOn,wWho);if(!wr.ok)continue;R.muts.push(wWho+(wOn?" wears ":" removes ")+wr.item);}}},
{t:"OUTFIT",apply:function(text,R){var ot=text.match(/\[OUTFIT:([^|\]]+)\|([^\]]+)\]/g)||[],oi;for(oi=0;oi<ot.length;oi++){var om=ot[oi].match(/\[OUTFIT:([^|\]]+)\|([^\]]+)\]/);if(!om)continue;var oWho=om[1].trim(),oCs=attireSheet(oWho);
  if(!oCs){if(typeof console!=="undefined")console.warn("[attire] [OUTFIT:"+oWho+"] — no party member by that name; ignored (#388)");continue;}
  var oo=outfitSet(oCs,om[2],R.turn);if(oo)R.muts.push(oWho+" outfit: "+oo.text);}}},
...
function applyMutsTable(text,opts){
  ...
  if(R.timeText&&typeof clockReconcilePhase==="function"){
    ...
    else if(worldState.reconcileSkip&&worldState.reconcileSkip.turn===R.turn)R.muts.push("Clock reconcile SKIPPED — '"+R.timeText+"' is "+Math.round(worldState.reconcileSkip.delta/60)+"h ahead "+(worldState.reconcileSkip.sameDay?"on the same day":"across dawn")+"; mislabel presumed, heal note queued (#142/#368)");
  }
